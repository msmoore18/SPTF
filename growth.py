...
def project_tree_growth(data, years=10):
    ...
    for year in range(0, years + 1):
        projection[f'Year {2025 + year}'] = projection['Tree Height (ft)'] + year * projection['Tree Height (ft)'].apply(lambda x: 1 if x > 2 else 0.5)
    return projection
...
def update_with_new_trees(projection, years=10, new_trees_per_year=0):
    ...
    for year in range(0, years + 1):
        new_tree_entries.append({
            ...
            'Row': 'N/A',
            'Quality': 'N/A',
            'Count': 4000 if year == 0 else new_trees_per_year
        })
...
def create_summary(projection, years=10):
    summary = projection.groupby("Tree Height (ft)")[[f'Year {2025 + i}' for i in range(0, years + 1)]].sum().reset_index()
    return summary
